...
func (device *Device) IpcSetOperation(r io.Reader) (err error) {
	...
	for scanner.Scan() {
		...
		if key == "public_key" {
			...
			err := device.handlePublicKeyLine(peer, key, value)
			if err != nil {
				return err
			}
			continue
		...
		var err error
		if deviceConfig {
			err = device.handleUpdateDeviceLine(key, value)
		} else {
			err = device.handleUpdatePeerLine(peer, key, value)
		}
		if err != nil {
			return err
		}
...
func (device *Device) handleUpdateDeviceLine(key, value string) error {
	switch key {
	case "private_key":
...
func (device *Device) handlePublicKeyLine(peer *ipcSetPeer, key, value string) error {
...
func (device *Device) handleUpdatePeerLine(peer *ipcSetPeer, key, value string) error {
	switch key {
	case "update_only":
